...
### Multiple subscribers: one bot, several phones

The Telegram layer was first built around a single operator. The push notifier — the component that announces a newly ingested invoice or a reconciliation outcome — sent each message to one destination: the first chat identifier on the whitelist. This was an unexamined assumption rather than a deliberate constraint, and testing with two phones exposed it immediately. Only the first phone received the state-change notifications; the second, though able to query the agent, never saw a push.

The distinction that surfaced here is worth stating precisely, because it separates two responsibilities that had been conflated. Inbound queries and outbound notifications are not symmetric. A query reply is a response to a specific sender and is correctly routed back to whoever asked — the bot answers the phone that called it, and this already worked for every whitelisted number. A notification, by contrast, is not a reply to anyone; it is an unsolicited announcement of a state change in the system, and the question of who should receive it is a policy decision, not a routing one. The original code answered that policy question implicitly and wrongly, by sending to a single hard-coded position in the whitelist.

The corrected design treats the whitelist as the subscriber list. When the system has something to announce — a new bill has arrived, a payment has been matched, a payment could not be matched — it broadcasts the message to every whitelisted chat, iterating the full set rather than selecting the first. Each send is independent and individually guarded: a failure to deliver to one recipient is logged and the loop continues, so one blocked or invalid recipient can neither suppress delivery to the others nor disturb the ingest and reconciliation pipeline that triggered the notification. The notifier remains, as before, a projection of state onto a user-facing surface that holds no agent reasoning of its own; the only change is that the projection now has several targets instead of one.

The practical consequence is that the system supports several subscribers to the same bot with no additional infrastructure. A household in which more than one person is responsible for the bills — two partners, or several family members sharing utility accounts — can each message the bot once to discover their chat identifier, have those identifiers added to the whitelist, and from that point forward all of them receive the same notifications and all of them can query the shared financial state. The whitelist thus serves a dual role: it is both the trust boundary, admitting only known participants to a world-discoverable bot, and the distribution list, defining the audience for every announcement. For the single-user thesis demonstration this is invisible, but it generalises the system from a personal tool to a small shared one without a schema change or a second moving part.

One refinement was considered and deliberately deferred: separating the right to query from the right to receive notifications, so that an occasional participant could ask the bot questions without being subscribed to every push. This would split the single whitelist into two lists — those allowed to query and those who receive broadcasts. It is a reasonable distinction for a larger deployment, but at the scale this system targets it adds configuration surface for a need that has not yet appeared, and the single-list design is retained until it does.
